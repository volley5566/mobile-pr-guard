...
## 24. 误报抑制:为什么这是"留得住"的关键?

**比喻**:一个总在你没违章时也叫的警报器,你三天就把它拆了。
review 工具同理——**误报却没法消音,是它被团队卸载的头号原因**。
工具再聪明,也一定有误报;能不能"优雅地闭嘴",决定了团队留不留它。

我们给两种消音方式(在所有扫描器跑完、统一过滤一遍):

1. **行内注释 `mpg-ignore`**:在问题行末尾或上一行写一下,只静音那一行。
   这跟你熟悉的 `// eslint-disable-line`、`# noqa`、`@SuppressWarnings` 是同一个思路——
   **让开发者在最了解上下文的地方,自己做"这是误报"的判断**。
2. **配置 `suppress`**:按 规则 + 路径(glob)整片静音,适合"这个遗留目录我们认了"。

实现要点(`src/suppress.py`):
- 行内标记靠 `added_lines_with_lineno()` 找到带 `mpg-ignore` 的行,
  静音"本行"和"下一行"两种写法;
- 配置抑制用规则**前缀**匹配(`semgrep` 命中 `semgrep:xxx`)+ 路径 glob;
- 被抑制多少条会打印出来,做到"消了音但有记录",不会偷偷吞掉。

> 产品层面的道理:**信任 = 命中率 - 误报代价**。提高命中靠加规则,
> 降低误报代价靠"一键消音"。后者往往比前者更影响留存——
> 这也是为什么在堆新功能之前,先把消音做好。
